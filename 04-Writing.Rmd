...
The concept of literate programming (*literate programming*) was developed by @Knuth1984.
...
The language for formatting the text is Markdown[^rediger-1], a simple to use markup language:
...
The equations are written in LaTeX format[^rediger-4].
...
The document contains Markdown formatted text and code chunks (*code chunks*) surrounded by three grave accents (the Markdown syntax of a code block) and a language description, here `r`.
...
Processing an R Markdown document is called *knit*.
...
It can be scrolled to choose the output format: HTML, Word, PDF (via LaTeX) and, for notepads, a "Preview" command that displays the document in HTML without executing the code snippets to save time.
...
Replace the vertical bars `|` in the above example with quotation marks: since this document is written with R Markdown, the date would be calculated and displayed instead of the code if the quotation marks were used directly.
...
The header of the code snippet of the figure `fig@ref(fig:pressure) is:
...
(ref:pressure2) Title with _italic_, math ($\sqrt\pi$) and link to figure \@ref(fig:pressure)
```{r pressure2, fig.cap="(ref:pressure2)", echo=FALSE}
plot(pressure)
```

The text in `fig.cap`, "Title of figure" previously, is replaced by `(ref:pressure)` _within the quotation marks_ which are retained and the caption is entered in a paragraph starting with `(ref:pressure)` followed by a space.
...
The horizontal - and vertical separators | allow to draw an array according to the Markdown syntax, but it is not the best method.
...
The caption is specified by the `caption` argument and referencing is possible because the array is given a label whose name is `tab:` followed by the name of the code snippet (array \@ref(tab:kable)).
...
Finally, the `full_width = FALSE` option allows to adjust the width of the table to its content instead of occupying the whole available width.

The **flextable** package allows creating more elaborate arrays, as in the following example which displays the long sepals in color.
...
iris %>%
  ...
  flextable() %>%
  # Titre des colonnes
  set_header_labels(Sepal.Length="Longueur sépales",
                    Sepal.Width="Largeur", 
                    Petal.Length="Longueur pétales",
                    Petal.Width="Largeur",
                    Species="Espèce") %>%
  # Sélection des longs sépales (>5) et affichage en rouge
...
Sections can be given a label by ending their title with `{#yyy}`.
...
- Format: "Better BibTeX" for articles and presentations or "Better BibLaTeX" for dissertations, depending on whether the bibliography is managed by bibtex and natbib or biber and biblatex for PDF production.
...
The references can be called in the text, between brackets by the code `[@Reference]`, or in the text, by removing the brackets.
...
In the organization proposed in section \@ref(sec:solution-folders), the parent folder is `%LOCALAPPDATA%\ProjectsR`.
...
#### Put online {#sec:article-en-ligne}

The **memoiR** package simplifies the uploading of produced documents.
...
The title levels allow you to separate the parts of the presentation (`#`) and the slides (`##`).
...
Run `build_git()` and `build_readme()`, enable source control and push the project to GitHub, in the same way as for an article (section \@ref(sec:article-in-line)).
...
The first chapter is placed in the foreword of the printed book: it should not be numbered (hence the `{-}` code next to the title) in the HTML version.
...
To break formatted text (between asterisks for italics or more frequently between inverted apostrophes for code), you must finish formatting before `\break` and start again afterwards.
...
The site template can easily be enhanced by adding `_site.yml`:

- by adding a GitHub icon in the navigation bar to link to the site source code:
- by choosing the method of knitting the pages, to use **bookdown** instead of **rmarkdown**;
- by placing the site files in the `docs` folder and thus separating code and production.
...
A Hugo site can be hosted for example on the personal page of any GitHub user whose address is of the form "GitHubID.github.io".
...
The repository can be the one of the main site of your GitHub account (see section \@ref(sec:github-pages)), at the address https://GitHubID.github.io[^rediger-40].
...
If the site is multilingual (French and English for example), its content (`content` folder) must be copied in a folder corresponding to each language.
For example, the `content/authors/admin/_index.md` file which contains information about the owner of the site is replaced by `content/en/authors/admin/_index.md` and `content/fr/authors/admin/_index.md` if the site supports English and French.
In practice, create an `en` folder and a `fr` folder in `content`.
Copy all the content of content (except the two new folders) into `en` and then move the same content into `fr`.
...
In "Basic Info", the linear sujet, par exemple "Theme" pour l'apparence générale.
...
The `edit_page` line needs to be updated: replace the default repository "<https://github.com/gcushen/hugo-academic>" with the one for the site.
...
The `posts` component fetches its content from the `/contents/<language>/post` folder which contains the blog posts (see below).
...
Finally, the `contact` component allows to display a contact form.
...
It is set up in `config/_default`\break`/menus.toml`.
...
The menu must be translated into each language in the file `config/_default/languages.toml`:
...
If a `featured.png` (optimal for a schematic) or `featured.jpg` (optimal for a photo) image is placed in the folder, it will be used as the thumbnail of the post.
...
The optimized functions are `agg_png()`, `agg_jpeg()` and `agg_tiff()`. Their usage is the same as that of the **grDevices** functions.
...
The stream is managed by code snippets of type `targets`.
...
The first code chunk, with the `tar_globals=TRUE` option, writes the global options for the stream. 
To create the stream shown in section `@ref(sec:targets), the code is simply:
...
During knitting, this simplified code (`tar_simple=TRUE`) is automatically transformed into a target write:
...
`tar_visnetwork()` is used to check that the stream is correct before running it.
...
This way, calculations done locally will not be repeated by GitHub Actions (chapter \@ref(chap-this)) and the document build will be fast.
