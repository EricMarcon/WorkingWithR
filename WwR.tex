...
It is recommended to use the ZotMoov\footnote{\url{http://https://github.com/wileyyugioh/zotmoov/}} extension to better control the location of the full text (PDF files linked to bibliographic references).
...
##  f1(X)   1.845   6.4985  11.5825   7.1545   7.8310  430.869
##  f2(X)   4.223   8.4460  17.8432   9.3480  10.1680  433.985
##  f3(X) 101.229 105.9235 125.0898 108.2400 110.9255  717.582
##  f4(X) 136.530 139.9125 213.4296 142.7415 157.0095 1184.572
##  neval
##    100
##    100
##    100
...
##                                    expr   median
## 1                              sqrt(x1)   1.5580
## 2                lapply(x1, FUN = sqrt) 141.9830
## 3                sapply(x1, FUN = sqrt) 176.0335
## 4 vapply(x1, FUN = sqrt, FUN.VALUE = 0) 138.3955
\end{verbatim}
...
\begin{verbatim}
##                        expr   median
## 1 vapply(x1, FUN = sqrt, 0) 143.8075
## 2                  Loop(x1)  44.2595
\end{verbatim}
...
\begin{verbatim}
##                                             expr   median
## 1                      vapply(x1, FUN = sqrt, 0)  164.697
## 2 for (i in 1:length(x1)) Root[i] <- sqrt(x1[i]) 2278.226
\end{verbatim}
...
\begin{verbatim}
##                        expr   median
## 1 replicate(1000, runif(1)) 995.3775
## 2               runif(1000)   7.2570
\end{verbatim}
...
\begin{verbatim}
##                              expr   median
## 1       SumLoop(diamonds[-(2:4)]) 2.081303
## 2 apply(diamonds[-(2:4)], 2, sum) 4.907064
## 3       colSums(diamonds[-(2:4)]) 1.590595
\end{verbatim}
...
##  550.22 573.385 903.6777 598.477 795.8305 8596.757   100
\end{verbatim}
...
This time is to be compared to that of the reference for loop, multiplied by 2500, that is \texttt{2070} seconds.
...
## ● completed target NbPoints [0.793 seconds, 53 bytes]
## ▶ dispatched target X
...
\begin{verbatim}
## ▶ dispatched target NbPoints
## ● completed target NbPoints [0.813 seconds, 53 bytes]
## ▶ dispatched target X
## ● completed target X [0.001 seconds, 11.058 kilobytes]
## ▶ dispatched target d
## ● completed target d [0.031 seconds, 55 bytes]
## ▶ dispatched target map
## ● completed target map [0.017 seconds, 187.39 kilobytes]
## ▶ ended pipeline [0.966 seconds]
\end{verbatim}
...
##  triple.integer(2L)  82 123 7348.02    123 164 720206   100
##   triple.numeric(2) 123 123 9842.87    123 164 969773   100
##          triple(2L) 492 533  603.52    533 574   4510   100
\end{verbatim}
