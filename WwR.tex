...
##  f1(X)  27.721  29.4455  41.95994  30.1985  31.9325
##  f2(X)  39.478  40.6170  50.46290  41.2015  42.2735
##  f3(X) 244.909 246.9255 260.19375 247.8840 250.1325
##  f4(X) 311.133 316.2215 333.12125 317.0680 319.7645
##       max neval
##  1088.811   100
##   881.235   100
##  1102.706   100
...
##                                    expr   median
## 1                              sqrt(x1)   7.2555
## 2                lapply(x1, FUN = sqrt) 269.3740
## 3                sapply(x1, FUN = sqrt) 310.6195
## 4 vapply(x1, FUN = sqrt, FUN.VALUE = 0) 280.9425
\end{verbatim}
...
\begin{verbatim}
##                        expr   median
## 1 vapply(x1, FUN = sqrt, 0) 285.1410
## 2                  Loop(x1)  71.3445
\end{verbatim}
...
\begin{verbatim}
##                                             expr   median
## 1                      vapply(x1, FUN = sqrt, 0)  282.950
## 2 for (i in 1:length(x1)) Root[i] <- sqrt(x1[i]) 2224.968
\end{verbatim}
...
\begin{verbatim}
##                        expr   median
## 1 replicate(1000, runif(1)) 1897.088
## 2               runif(1000)   15.068
\end{verbatim}
...
\begin{verbatim}
##                              expr   median
## 1       SumLoop(diamonds[-(2:4)]) 2.880943
## 2 apply(diamonds[-(2:4)], 2, sum) 7.195426
## 3       colSums(diamonds[-(2:4)]) 2.006073
\end{verbatim}
...
This is knitted with \texttt{4} cores, all of which are used except for one so as not to saturate the system.
...
Setting up parallelization has a cost of about \texttt{0.025} seconds here.
...
\caption[Execution time in parallel]{Parallel execution time of tasks requiring one second (each task is a one second pause). The number of tasks varies from 1 to twice the number of cores used (equal to \texttt{3}) plus one.}\label{fig:r-parallele}
\end{figure}
...
\NormalTok{  distances }\OtherTok{\textless{}{-}} \FunctionTok{foreach}\NormalTok{(}
    \AttributeTok{i =} \FunctionTok{icount}\NormalTok{(Y}\SpecialCharTok{$}\NormalTok{n), }
...
##  279.545 300.6735 346.5516 318.7385 324.533 1721.42   100
\end{verbatim}
...
This time is to be compared to that of the reference for loop, multiplied by 2500, that is \texttt{3864} seconds.
...
## ● completed target NbPoints [1.654 seconds]
## ▶ dispatched target X
## ● completed target X [0.004 seconds]
## ▶ dispatched target d
## ● completed target d [0.018 seconds]
## ▶ dispatched target map
## ● completed target map [0.143 seconds]
## ▶ completed pipeline [2.001 seconds]
\end{verbatim}
...
\begin{verbatim}
## ▶ dispatched target NbPoints
## ● completed target NbPoints [1.687 seconds]
## ▶ dispatched target X
## ● completed target X [0.004 seconds]
## ▶ dispatched target d
## ● completed target d [0.02 seconds]
## ▶ dispatched target map
## ● completed target map [0.028 seconds]
## ▶ completed pipeline [1.96 seconds]
\end{verbatim}
...
## Unit: nanoseconds
##                expr  min     lq     mean median     uq
##  triple.integer(2L)  295  305.0 24060.71    309  318.0
##   triple.numeric(2)  293  307.5 22323.16    319  327.5
##          triple(2L) 1067 1083.5  1182.95   1092 1157.0
##      max neval
##  2374390   100
##  2200039   100
##     5537   100
...
\hypertarget{sec:package-pr-ci6}{%
