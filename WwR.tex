...
Two methods are available: HTTPS (also known as SSL) and SSH.
SSH is the most robust, but requires the creation of a private key.

Display RStudio's global options (\enquote{Tools \textgreater{} Global Options\ldots{}} menu): select \emph{Git/SVN} then click on the \enquote{Create SSH Key\ldots{}} button and choose the \enquote{ed25519} key type.
You can add a password (\emph{passphrase}) to the key, which must be typed in the first time each session is used.
If the computer is properly secured (no physical access by third parties), leaving it empty will increase fluidity.

The private key is stored in the `.ssh' folder of the user's home directory.
The corresponding public key must be uploaded to GitHub: click on the \enquote{View public key} link and copy the key to the clipboard.

On GitHub, display your account settings (\enquote{Settings} menu), select \enquote{SSH and GPG Keys}, click on \enquote{New SSH Key} and paste the public key into the \enquote{Key} field.
Give the key a name in the \enquote{Title} field.
The name can be that of the computer on which the key was created.

Warning: the private key is strictly confidential and must not be copied anywhere where it could be read by a third party (be careful with automatic backups in particular).
It does not need to be backed up: if lost, it can be easily replaced.
The key must not be copied to multiple computers: if necessary, create a new key on each workstation used.

In the RStudio terminal, check that the key is working properly:

\begin{verbatim}
ssh -T git@github.com
\end{verbatim}

If successful, a message will indicate that the authenticity of the GitHub server cannot be verified: a manual check is required for the first connection.
Check with GitHub that the server fingerprint is correct\footnote{\url{https://docs.github.com/en/github/authenticating-to-github/githubs-ssh-key-fingerprints}} and type \texttt{yes}.
The server is automatically added to the list of known servers in the \texttt{known\_hosts} file.

If the key is compromised (lost or the computer containing it is loaned to someone else), delete it from GitHub and create a new one.
...
##  f1(X)   2.173   7.2365  16.82476   8.2205  14.5550
##  f2(X)   4.346   8.9790  17.29503  10.5165  13.9195
##  f3(X)  97.580 105.7595 123.53505 109.4495 119.2690
##  f4(X) 129.601 138.6415 170.65020 143.9100 162.7290
##       max neval
##   459.774   100
##   478.142   100
##   627.095   100
...
##                                    expr  median
## 1                              sqrt(x1)   3.034
## 2                lapply(x1, FUN = sqrt) 140.876
## 3                sapply(x1, FUN = sqrt) 175.439
## 4 vapply(x1, FUN = sqrt, FUN.VALUE = 0) 139.728
\end{verbatim}
...
\begin{verbatim}
##                        expr   median
## 1 vapply(x1, FUN = sqrt, 0) 133.0245
## 2                  loop(x1)  42.3735
\end{verbatim}
...
\begin{verbatim}
##                                             expr   median
## 1                      vapply(x1, FUN = sqrt, 0)  136.448
## 2 for (i in 1:length(x1)) root[i] <- sqrt(x1[i]) 1207.696
\end{verbatim}
...
\begin{verbatim}
##                        expr  median
## 1 replicate(1000, runif(1)) 720.411
## 2               runif(1000)   6.027
\end{verbatim}
...
\begin{verbatim}
##                              expr   median
## 1      loop_sum(diamonds[-(2:4)]) 1386.579
## 2 apply(diamonds[-(2:4)], 2, sum) 3183.896
## 3       colSums(diamonds[-(2:4)]) 1228.995
\end{verbatim}
...
Setting up parallelization has a cost of about \texttt{0.081} seconds here.
...
##  220.58 233.8025 293.2427 240.2805 251.8425 3356.834   100
\end{verbatim}
...
This time is to be compared to that of the reference for loop, multiplied by 2500, that is \texttt{1989} seconds.
...
\begin{verbatim}
## + n_points dispatched
## ✔ n_points completed [696ms, 53 B]
## + X dispatched
## ✔ X completed [2ms, 11.06 kB]
## + d dispatched
## ✔ d completed [3ms, 55 B]
## + map dispatched
## ✔ map completed [12ms, 187.83 kB]
## ✔ ended pipeline [849ms, 4 completed, 0 skipped]
\end{verbatim}
...
\begin{verbatim}
## ✔ skipped pipeline [34ms, 4 skipped]
\end{verbatim}
...
The owner of the project is added from his public key stored on GitHub, which is the second one in our example.
...
\FunctionTok{add\_github\_user}\NormalTok{(github\_user, }\AttributeTok{vault =}\NormalTok{ vault, }\AttributeTok{i =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
## + n_points dispatched
## ✔ n_points completed [669ms, 53 B]
## + X dispatched
## ✔ X completed [1ms, 11.06 kB]
## + d dispatched
## ✔ d completed [4ms, 55 B]
## + map dispatched
## ✔ map completed [12ms, 187.83 kB]
## ✔ ended pipeline [845ms, 4 completed, 0 skipped]
\end{verbatim}
...
\begin{verbatim}
## [1] "plot.AccumCurve"      "plot.acf"            
## [3] "plot.ACF"             "plot.adaptivedensity"
## [5] "plot.addvar"          "plot.anylist"
\end{verbatim}
...
##  triple.integer(2L) 123 123  150.88    123 164    451   100
##   triple.numeric(2) 123 123 8488.23    123 164 832710   100
##          triple(2L) 492 492 7132.36    533 533 658747   100
\end{verbatim}
...
A GitHub organization basically contains members (GitHub account holders) and repositories that can be accessed at \texttt{https://github.com/\textless{}Organization\textgreater{}/\textless{}RepoID\textgreater{}}.
