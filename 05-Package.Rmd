...
This "import" simply means that the **stats** package must be loaded, but not necessarily attached (see section `@ref(sec:environments)), for **multiple** to work.
...
This is not a good practice because it multiplies the risks of name conflicts (see section \@ref(sec:environments)).
...
Otherwise, the package check returns a note that the variables `x` and `y`, used by the arguments to `aes()` have not been declared and may not exist in the local environment (see section \@ref(sec:environments)).
...
Example[^507]: documentation of the **SpatDiv** package hosted on GitHub, `.R` file of the package:
...
The email address of the package maintainer (*maintainer*) must remain valid and the requests must be processed quickly. 
