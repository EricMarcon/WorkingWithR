...
The repository address is `https://github.com/GitHubID/NomDuDepot.git` if HTTPS authentication is chosen.
...
knitr::include_graphics('images/git-historic.png')
```
...
- Enter the name of the key: `ProjectName.rsa`.
- Do not enter a passphrase to allow the key to be used without interaction.

The private key `ProjectName.rsa` should only be distributed to the rightful owners of the project.
...
Apply the procedure in the section \@ref(sec:secrets-ci) to create a secret named "RSA" and paste the content of the file `ProjectName.rsa` in the "Value" field of the form.
